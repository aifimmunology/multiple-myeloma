...
# %% [markdown]
# # 2025-09-08: PBMC - Run scNMF on CD4 T cells - BRI Cells
...
# %%
import NMFproj
print(NMFproj.__version__)

# %%
import resource

# Set memory limit (bytes) - 300GB
memory_limit_gb = 300
...
print(f"Memory limit set to: {current_limit[0] / (1024**3):.0f} GB")
...
adata = adata[adata.obs['manual.category']
              == 'healthy_pbmc']  # Keep only BRI T cells

adata.raw = adata  # save in raw adata

# Scanpy Normalize to avoid sum step using the matrix
sc.pp.normalize_total(adata, target_sum=1e4, copy=False)
sc.pp.log1p(adata, copy=False)

adata.write('../../../data/rna/pbmc-subsets/pbmc-t-cd4-bri-input-nmf.h5ad')

# %%
adata = sc.read_h5ad('../../../data/rna/pbmc-subsets/pbmc-t-cd4-bri-input-nmf.h5ad')
...
# %%
## Run the NMFproj function; here, I get a weird flag even though I pass normalized = True
...
# %%
adata.write('../../../data/rna/pbmc-subsets/pbmc-t-cd4-bri-nmf-raw.h5ad')
adata.obs.to_parquet('../../../data/rna/pbmc-subsets/pbmc-t-cd4-bri-nmf-metadata.parquet')
...
adata.write('../../../data/rna/pbmc-subsets/pbmc-t-cd4-bri-nmf-processed.h5ad')
